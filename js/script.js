document.addEventListener('DOMContentLoaded', () => {
    ...
    const fallbackConfig = {
        ...
        "inbounds": [
            {"tag": "socks", "port": 10808, "listen": "127.0.0.1", "protocol": "socks", "sniffing": {"enabled": true, "destOverride": ["http", "tls"]}, "settings": {"auth": "noauth", "udp": true, "userLevel": 8}},
            {"tag": "http", "port": 10809, "listen": "127.0.0.1", "protocol": "http", "sniffing": {"enabled": true, "destOverride": ["http", "tls"]}, "settings": {"userLevel": 8}}
        ],
        ...
        "routing": {
            ...
            "rules": [
                ...
                {"type": "field", "outboundTag": "direct", "domain": ["domain:ir", "geosite:ir"]},
                {"type": "field", "outboundTag": "block", "domain": ["geosite:category-ads-all"]},
                {"type": "field", "outboundTag": "direct", "ip": ["geoip:ir", "geoip:private"]},
                {"type": "field", "outboundTag": "chain1-fragment", "network": "tcp"},
        ...
        "outbounds": [
            ...
            {"tag": "chain1-fragment", "protocol": "freedom", "settings": {"fragment": {"packets": "tlshello", "length": "100-200", "interval": "10-20"}}, "streamSettings": {"sockopt": {"tcpKeepAliveIdle": 100, "mark": 255}}}
        ]
    };
    ...
    function parseJsonc(jsoncString) {
        try {
            ...
            console.error('JSONC Parse Error:', error);
            throw error;
        }
    }
    ...
    if (generateButton) {
        generateButton.addEventListener('click', async () => {
            const jsonInput = jsonConfigInput ? jsonConfigInput.value.trim() : '';
            const routeAll = routeAllCheckbox ? routeAllCheckbox.checked : false;
            let userConfig;
            ...
            if (!jsonInput) {
                if (outputJson) outputJson.innerHTML = 'Error: Please enter a JSON config.';
                return;
            }
            ...
            try {
                ...
                if (outputJson) outputJson.innerHTML = 'Error: Input is not a valid JSON or JSONC. Please check the config format.';
                return;
            }

            if (!userConfig.outbounds || !userConfig.routing) {
                if (outputJson) outputJson.innerHTML = 'Error: Input config is incomplete. The `outbounds` and `routing` sections are required.';
                return;
            }
            ...
            if (isLoadBalanced) {
                ...
                if (!userBalancer) {
                    if (outputJson) outputJson.innerHTML = `Error: Load-balanced config must contain a balancer with the tag "${mainBalancerOriginalTag}".`;
                    return;
                }
            } else {
                const singleProxy = userConfig.outbounds.find(o => o.tag === singleProxyOriginalTag);
                if (!singleProxy) {
                    if (outputJson) outputJson.innerHTML = `Error: Single config must contain a primary outbound with the tag "${singleProxyOriginalTag}".`;
                    return;
                }
            }

            const routeItems = ipInput ? ipInput.value.split('\n').map(item => item.trim()).filter(item => item) : [];
            if (routeItems.length === 0 && !routeAll) {
                if (outputJson) outputJson.innerHTML = 'Error: The IP/Domain list cannot be empty when "Route All Traffic" is unchecked.';
                return;
            }
            ...
            try {
                ...
                if (outputJson) outputJson.innerHTML = `Error: ${error.message}`;
                return;
            }
            ...
            try {
                ...
                if (outputJson) outputJson.dataset.rawjson = finalJsonStringToCopy;
                ...
                setTimeout(() => { 
                    if (outputJson) outputJson.innerHTML = finalHtml; 
                }, 500);
            } catch (error) {
                setTimeout(() => { 
                    if (outputJson) outputJson.innerHTML = `Error processing config: ${error.message}\nPlease check the input format.`; 
                }, 500);
            }
        });
    }
    ...
    if (copyButton) {
        copyButton.addEventListener('click', () => {
            const textToCopy = outputJson ? outputJson.dataset.rawjson : '';
    ...
    if (clearButton) {
        clearButton.addEventListener('click', () => {
            ...
            setDefaultIPs();
            if (routeAllCheckbox) routeAllCheckbox.checked = false;
            if (outputJson) {
